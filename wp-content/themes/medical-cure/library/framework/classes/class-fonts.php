...
* Usage:
*
* For getting a list of all fonts use:
* $c5_font_obj = new C5_font();
* $options = $c5_font_obj->get_custom_fonts_options($section);
* ------------
* For hooking the main global variable for the theme
* global $c5_font_obj;
* $c5_font_obj = new C5_font();
* $c5_font_obj->hook();
* $c5_font_obj->set_fonts( 'heading_font', $c5_skindata['heading_font']  );
* $c5_font_obj->set_fonts( 'body_font', $c5_skindata['body_font']  );
* ------------
* For assiging the fonts in custom css
* global $c5_font_obj;
* $heading_font = $c5_font_obj->get_font_name('heading_font');
* $body_font = $c5_font_obj->get_font_name('body_font');
* ------------
* For adding fonts via Filter
* add_filter('c5_add_fonts', 'c5_add_extra_fonts');
* function c5_add_extra_fonts($fonts) {
* 	$fonts[] =  array(
* 			'label'=>'',
* 			'family'=>'',
* 			'url'=> ''
...
class C5_font {
	...
	public function set_fonts($key ,$value='')
	{
		$this->applied_fonts[$key] = $value;
	}

	public function get_fontface_fonts() {

		// name of transient in database
		$transName = 'code125-fontface-fonts';
		...
		if( $content === false ) {
			...
			$fontfaceApi = 'https://fontface.me/font/all';
	...
	public function get_font_name($key)
	{
		if (isset(	$this->applied_fonts[$key] )) {
			$font_data = explode('#', $this->applied_fonts[$key] );
			if (!isset( $font_data[2] ) ) {
				return;
			}
			if ($font_data[2]=='earlyaccess') {
	...
	{
		$main_fonts = array_unique($this->applied_fonts);



		foreach ($main_fonts as $key => $value) {
	...
	public function enqueue($key , $value)
	{
		...
		if (strpos($key, 'rtl') !== false && !is_rtl()) {
			return;
		}
		$info = explode('#', $value);
		if(!isset( $info[2] )){
		...
		if($info[2]=='googlefont'){
		...
		}elseif($info[1]=='fontface'){
			wp_enqueue_style( 'c5-font-'. $key, $http . '"www.fontstatic.com/f='.$info[0]);
			...
			if(is_array($fonts)){
				foreach ($fonts as $font) {
					if($font['name'] == $info[0]){
						if ($font['url'] != '') {
							wp_enqueue_style( 'c5-font-'. str_replace(" ", "_", $font['name']), $font['url']);
						}

					}
				}
			}
		}elseif ($info[2]=='customplugin') {
			$plugin_fonts = array();
			$plugin_fonts = apply_filters( 'c5_add_fonts',$plugin_fonts);

			if (!empty($plugin_fonts)) {
				foreach ($plugin_fonts as $font) {
					if ($font['family'] == $info[0] ) {
						wp_enqueue_style( 'c5-font-' . $font['family']  ,  $font['url'] );
					}
				}
			}
			wp_enqueue_style( 'c5-font-' . $info[0]  ,  $info[3] );
		}
	}


	public function get_custom_fonts_options($section='')
	{
		$options = array(
			array(
				'label' => 'Custom Fonts',
				...
				'settings' => array(
					array(
						...
						'id' => 'name',
						'type' => 'text',
						'desc' => 'Font Name to be added in the CSS "without " or \' "',
						'std' => '',
					),
					array(
						'label' => '.css file url',
						'id' => 'url',
						'type' => 'text',
						'desc' => 'The Url for css file that contains the data about the font. Incase you are adding fonts via plugins, then you should leave this field empty.',
						'std' => '',
					)
				),
				'std' => '',
	...
	public function get_arabic_fonts()
	{
		$this->init_fonts();
		...
		$fontface =  $this->get_fontface_fonts();
		if (is_array($fontface)) {
			foreach ($fontface as $fontface_single) {
				$array[] = array(
					'label'=>$fontface_single['name'] ,
					'value'=>$fontface_single['permalink'] . '#fontface#default'
				);
			}
		}
		...
		if ($custom_fonts) {
			foreach ($custom_fonts as $custom_font) {
				$array[] = array(
					'label'=>$custom_font['name'] . ' (Custom Font)',
					'value'=>$custom_font['name'] . '#system#custom'
				);
			}
		}
		...
		if (!empty($plugin_fonts)) {
			foreach ($plugin_fonts as $plugin_font) {
				$array[] = array(
					'label' => $plugin_font['label'],
					'value' => $plugin_font['family'] . '#' . $plugin_font['family'] . '#customplugin#custom'
				);
			}
		}
