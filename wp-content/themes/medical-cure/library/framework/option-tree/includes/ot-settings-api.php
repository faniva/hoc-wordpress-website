    ...
    foreach( (array) $this->options as $option ) {
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
            ...
            if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
                ...
                echo '<div class="wrap about-wrap c5-admin-wrap settings-wrap" id ="page-' . $page['id'] . '">';

                do_action('c5_theme_options_header');
                ...
                if ( $show_buttons ) {
                    ...
                    echo '<button type="submit" class="option-tree-ui-button button button-secondary left reset-settings" title="' . __('Reset Options', 'medical-cure') . '">' . __('Reset Options', 'medical-cure') . '</button>';
