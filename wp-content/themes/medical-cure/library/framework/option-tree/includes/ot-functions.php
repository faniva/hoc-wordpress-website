...
if ( ! function_exists( 'ot_get_option' ) ) {
    ...
    function ot_get_option( $option_id, $default = '' ) {
		...
		if (isset($c5_overwrite_options[$option_id])) {
			return $c5_overwrite_options[$option_id];
		}



        /**
         * Get value from Customized pages
         */
        if (function_exists('c5_fw_get_custom_option')) {
            $value = c5_fw_get_custom_option($option_id);
            if ($value != '') {
                return $value;
            }
        }
        ...
        /**
         * Get value from skins
         */
        if (function_exists('c5_fw_get_option')) {
            $value = c5_fw_get_option($option_id);
            if ($value != '') {
                return $value;
            }
        }
        ...
        if ($default == '') {
            global $c5_theme_defaults;
            if(empty($c5_theme_defaults)){
                $c5_theme_defaults = get_option('c5_theme_defaults');
                if( !is_array($c5_theme_defaults) ){
                    $options = array();
            ...
            if (isset($c5_theme_defaults[$option_id])) {
                $default =  $c5_theme_defaults[$option_id];
            }
        ...
        if ( function_exists('icl_t') ) {
            ...
            if ( isset( $settings['settings'] ) ) {
                ...
                foreach( $settings['settings'] as $setting ) {
                    ...
                    if( $option_id == $setting['id'] && $setting['type'] == 'button' ){

                        foreach( $options[$option_id] as $ckey => $cvalue ) {
                            $id = $option_id . '_' . $ckey;
                            $_string = icl_t( 'Theme Options', $id, $cvalue );

                            if ( ! empty( $_string ) ) {
                                $options[$option_id][$ckey] = $_string;
                            }
                        }
                    }
