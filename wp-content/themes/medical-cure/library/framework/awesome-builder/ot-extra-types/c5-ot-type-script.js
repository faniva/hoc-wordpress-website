jQuery(document).ready(function($) {
    ...
    C5AB_POSTS_SELECT_JS = {
        init: function() {
            ...
            $('.c5-post-type-quick').each(function handle_post_type_quick() {
                var this_obj = $(this);
                        ...
                        if (final_value!='') {
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    var append_obj = c5_get_closest_object(this_obj, 'ul.c5-current-values');
                                    append_obj.append('<li class="ui-state-default" data-info="' + final_value + '">' + data + '<span class="fa fa-times"></span></li>');


                                    var save_obj = c5_get_closest_object(this_obj, '.c5-save-post-type-value');
            ...
            $('.c5-posts-select-post-type').each(function handle_post_search() {
                var this_obj = $(this);
                this_obj.selectize({
                    valueField: 'value',
                    labelField: 'title',
            ...
            $('.c5-posts-search-post-type').each(function handle_post_search() {
                ...
                this_obj.selectize({
                    ...
                    onChange: function(post_type_name) {
                        if (post_type_name != '') {
                            $.ajax({
                                type: "post",
                                url: ajaxurl,
                                data: "action=c5_get_posts_by_search&post_type=" + post_type_name,
                                success: function(data) {
                                    var main_append_obj = c5_get_closest_object(this_obj, '.c5-articles-wrap');
    ...
    $(document).on('click', '.c5-icon-select', function(e) {
        var parent_object = $(this).parent();
        if (parent_object.children('.c5-icons-wrap').hasClass('c5-show-icons-wrap')) {
            parent_object.children('.c5-icons-wrap').removeClass('c5-show-icons-wrap')
        }else{
            parent_object.children('.c5-icons-wrap').addClass('c5-show-icons-wrap');
        }
        parent_object.children('.c5-icons-wrap').html($('.c5-icons-list-wrap-hidden').html());


    });
    $(document).on('click', '.c5_span_icon', function(e) {
        var parent_object = $(this).parent().parent();
        var current_value = $(this).attr('data-class');
        parent_object.children('.c5-icon-list-value').val(current_value);
        parent_object.children('.c5-icon-select').children('span').removeClass().addClass(current_value);
        parent_object.children('.c5-icon-select').children('span').html(current_value);

        parent_object.children('.c5-icons-wrap').removeClass('c5-show-icons-wrap');

    });

    $(document).on('click', '.c5-close-panel', function(e) {
        var parent_object = $(this).parent();
        parent_object.removeClass('c5-show-icons-wrap');
    ...
    $(document).on('keyup', 'input.c5-icons-search', function (e) {

        var text = $(this).val();

        var text_search = String(text);
        text_search.toLowerCase();
        var items = $(this).parent().children('.c5_span_icon');
        ...
        items.filter(function (e) {
            ...
            var icon_data =  String($(this).attr('data-class'));
             if( icon_data.toLowerCase().search(text_search) >= 0 ) {
                bool= true;
             }
             return bool;
        }).show();
