...
class C5_option_tree_type {
	...
	function __construct() {
		...
		define('C5_OT_EXTRA_TYPE_ROOT', C5BP_URI .'ot-extra-types/');
	...
	function hook_script_and_style() {
		...
		wp_enqueue_script( 'c5-ot-type-script-js', C5_OT_EXTRA_TYPE_ROOT.'c5-ot-type-script.js', '1.1', false );
...
if ( ! function_exists( 'ot_type_title' ) ) {
	function ot_type_title( $args = array() ) {
		...
		echo '<div class="format-setting type-title ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';

		/* description */
		echo $has_desc ? '<div class="c5-title"><h2>' . htmlspecialchars_decode( $field_desc ) . '</div></h2>' : '';
...
add_action('admin_notices' , 'c5_icons_list_main_copy');
function c5_icons_list_main_copy()
{
	$icons_obj = new C5AB_ICONS();
	$icons = $icons_obj->get_icons_html();

	echo '<div class="c5-icons-list-wrap-hidden" style="display:none;"><span class="fa fa-close c5-close-panel"></span><input type="text" name="" value="" placeholder="Search for an icon..." class="c5-icons-search" />'.$icons .'</div>';

	echo '<div class="c5-icons-wp-editor-wrap"><div class="c5-icons-wp-editor"><div class="c5-the-wp-editor-icons"></div></div></div>';
}
...
if ( ! function_exists( 'ot_type_icon_list' ) ) {
	...
	function ot_type_icon_list( $args = array() ) {
		...
		echo '<input type="hidden" name="' . esc_attr( $field_name ) . '" id="' . esc_attr( $field_id ) . '" value="'.$field_value.'" class="c5-icon-list-value" />';

		echo '<p class="c5-icon-select"><span class="'.$field_value.'">'.$field_value.'</span></p>';
...
if ( ! function_exists( 'ot_type_post_type_select' ) ) {
	...
	function ot_type_post_type_select( $args = array() ) {
		...
		echo '<p>Choose Post Type for your query</p>';
		echo '<select class="c5-post-type-quick">';
		...
		foreach ($post_types_all as $key => $post_type) {
			if (!in_array($key, $exlude_array_no_page)) {
				echo '<option value="'.$key .'">'.$post_type->label .' All Articles</option>';
			}
		}
		...
		foreach ($categories as $category) {
			...
			echo '<option value="post#category#'.$category->term_id .'">'.$category->cat_name . ' ('.$category->category_count.') </option>';
		}

		echo '</select>';

		//Choose Post Tyope
		echo '<p>Or choose Post Type:</p><select class="c5-post-type c5-screen-'.$screen_base.'" name="c5-post-type" id="c5-post-type">';
		$args = array('show_ui' => true);
		$output = 'objects'; // names or objects
		$post_types_all = get_post_types($args, $output);
		$exlude_array_no_page = array(
...
if ( ! function_exists( 'ot_type_posts_search' ) ) {
	...
	function ot_type_posts_search( $args = array() ) {
		...
		if ($field_post_type == '') {
			...
			$obj = get_post_type_object( $field_post_type );
			echo '<option value="'.$field_post_type .'">'.$obj->label .'</option>';

			//echo '<div class="c5-articles-wrap"><p>Search Staff</p><select name="c5-articles-select" id="c5-articles-select" class="c5-articles-select selectized" tabindex="-1" style="display: none;"><option value="" selected="selected"></option></select><div class="selectize-control c5-articles-select single"><div class="selectize-input items not-full focus input-active"><input type="text" autocomplete="off" tabindex="" style="width: 4px; opacity: 1; position: relative; left: 0px;" data-com.agilebits.onepassword.user-edited="yes"></div><div class="selectize-dropdown c5-articles-select single" style="display: none; width: 630px; top: 34px; left: 0px;"><div class="selectize-dropdown-content"></div></div></div></div>';
		}
