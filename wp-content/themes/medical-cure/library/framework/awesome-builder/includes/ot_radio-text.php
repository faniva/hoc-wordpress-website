...
if ( ! function_exists( 'ot_type_wp_editor' ) ) {
    ...
    function ot_type_wp_editor( $args = array() ) {
        ...
        if ($screen->base == 'widgets') {
            ot_type_textarea_simple($args);
            return;
        }

        if( check_ajax_referer( 'save-sidebar-widgets', 'savewidgets' ,false )){
            ot_type_textarea_simple($args);
            return;
        }
        ...
        /* format setting outer wrapper */
        echo '<style>body.widgets-php .c5-type-editor{display:none;}.c5-editor-sorry{display:none;} body.widgets-php .c5-editor-sorry{display:block; margin-bottom: 20px;}</style>';
        echo '<div class="c5-editor-sorry">Sorry, this element is available only in the Page Builder.</div>';
        echo '<div class="format-setting c5-type-editor type-textarea ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . ' fill-area">';
