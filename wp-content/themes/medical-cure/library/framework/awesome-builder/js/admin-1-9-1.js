...
;(function($) {
  C5AB_UI = {
    ...
    defaut_actions: function (){
        ...
        */
    	...
    	$(document).on('click', '.c5ab-launch-video', function (e) {
    		...
    		var data = '<iframe width="853" height="480" src="https://www.youtube.com/embed/' +$(this).attr('data-video')+ '?rel=0" frameborder="0" allowfullscreen></iframe>';
    	...
    	$(document).on('click', '.c5ab-load-import', function importTemplate (e) {
    		C5AB_UI.loadingWindow();
    		$.ajax({
    		    type: "post",
    		    url: c5ab_ajax_object.ajax_url,
    		    data: "action=c5ab_load_import_export&post_id=" + $(this).attr('data-post-id'),
    		    success: function(data){
    		        ...
    		        $.magnificPopup.open({
    		            ...
    		            mainClass: 'c5ab-widgets-post c5ab-import-window',
    		            type: 'inline',
    		            closeOnContentClick:false,
    	...
    	$(document).on('click', '.c5ab-import-button', function ImportCode (e) {

    		var c5_content = $('.c5ab-import-textarea').val();
    		C5AB_UI.loadingWindow();
    		var data = {
    				action: 'c5ab_template_import',
    				c5ab_content: c5_content,
    			};
        ...
        $(document).on('click', '.c5ab-add-quick-template', function AddThemeTemplates(e) {

    		C5AB_UI.loadingWindow();
    		var data = {
    				action: 'c5ab_load_theme_templates',
    			};
    		$.post(ajaxurl, data, function(response) {
                $.magnificPopup.close();
                $.magnificPopup.open({
                ...
                $('.code125-theme-templates-category').slick({
                    slidesToShow: 3,
                    slidesToScroll: 1,
        ...
        $(document).on('click', '.c5ab-add-page-template', function AddThemeTemplates(e) {

    		C5AB_UI.loadingWindow();
    		var data = {
    				action: 'c5ab_load_pages_templates',
    			};
    		$.post(ajaxurl, data, function(response) {
                $.magnificPopup.close();
                $.magnificPopup.open({
    			    ...
    			    mainClass: 'c5ab-widgets-post c5ab-page-templates-wrap',
    			    type: 'inline',
    			    closeOnContentClick:false,
    			    closeOnBgClick:false,
    			    showCloseBtn:false,
        ...
        $(document).on('click', '.c5ab-page-template', function load_theme_template(e) {
            $.magnificPopup.close();
    		C5AB_UI.loadingWindow();
            var data = {
    				action: 'c5ab_load_page_template',
        ...
        $(document).on('click', '.c5ab-theme-template ', function load_theme_template(e) {
            $.magnificPopup.close();
    		C5AB_UI.loadingWindow();
            var data = {
    				action: 'c5ab_load_theme_template',
    				id: $(this).attr('data-id'),
    			};
    		$.post(ajaxurl, data, function(response) {
    	...
    	$(document).on('click', '.c5ab-cache', function (e) {
            var this_obj = $(this);

            var val = this_obj.attr('data-cache');
            if (val == 'off') {
                val = 'on';
            }else{
                val = 'off';
            ...
            this_obj.parent().removeClass('c5ab-cache-off');
            this_obj.parent().removeClass('c5ab-cache-on');

            this_obj.parent().addClass('c5ab-cache-'+val);
            this_obj.parent().children('input').val(val);
        });
    	...
    	$(document).on('click', '.c5ab-control-panel span', function (e) {
            var this_obj = $(this);
            if (this_obj.hasClass('c5ab_preset_theme-template')) {
                C5AB_UI.loadingWindow();
        		var data = {
        				action: 'c5ab_load_theme_templates',
        			};
        ...
        $(document).on('click', '.c5ab-add-element .c5ab-edit-column-style', function (e) {

    		var id = $(this).parent().parent().parent().parent().attr('data-id');
            var data = {
    				action: 'c5ab_load_column_style',
    				id: id,
    				options: $('#c5ab-' + id + '-settings').val(),
    			};
    		C5AB_UI.loadingWindow();
    	...
    	$(document).on('click', '.c5ab-add-element .c5ab-add-new-element', function (e) {
    	...
    	$(document).on('click', '.c5ab-element ', function (e) {
            ...
            if ( e.srcElement.className.indexOf('c5ab-sq-btn') === -1 ) {
                C5AB_UI.launchWidgetEdit(new_id);
            }
    	});
        ...
        $(document).on('click', '.c5-row-tab-selector', function (e) {
    		var this_li = $(this);
            var this_content = $('.c5-content-' +  this_li.attr('data-tab'));
            var this_parent = this_li.parent().parent();
            this_parent.find('.c5-row-tabs-content').css('display','none');
            this_parent.find('.c5-row-tab-selector').removeClass('selected');
    ...
    updateColumnsHeight: function(){
        $('#c5bp-builder-panel .c5ab-layout-base-elements').css('min-height','68px');
        $('#c5bp-builder-panel .c5ab-layout-base-elements').each(function(){
            var this_obj = $(this);
            var this_parent = this_obj.closest('.c5ab-row-base-elements');
            height = this_parent.height() - 78;
            this_obj.css('min-height',height +'px');
    ...
    saveWidgetData: function (elm) {
        ...
        $(document).on('click', '.c5ab-save-widget-data-and-update', function (e) {
    		tinyMCE.triggerSave();
    		var t = $(this);
    		var data = {
    				action: 'c5ab_save_widget_data',
        ...
        $(document).on('click', '.c5ab-save-row-data-and-update', function (e) {
        ...
        $(document).on('click', '.c5ab-save-column-data', function (e) {

    		var t = $(this);
    		var data = {
    				action: 'c5ab_save_row_data',
    				content: $('#c5ab-column-form').serialize()
    			};
        ...
        $(document).on('click', '.c5ab-save-column-data-and-update', function (e) {
