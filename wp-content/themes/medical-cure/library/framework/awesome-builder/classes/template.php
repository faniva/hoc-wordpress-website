...
class C5PB_TEMPLATE extends C5PB_BASE{
	...
	function layout_css() {
		...
		<style>
		.c5ab-ruler-single{
			width: <?php echo 100/C5BP_COLUMNS_COUNT; ?>%;
		}
		<?php
		...
		for ($i = 1; $i <= C5BP_COLUMNS_COUNT; $i++) {
			...
			.c5ab-grid-<?php echo $i?>{
				left: <?php echo ($i*100)/C5BP_COLUMNS_COUNT ?>%;
			}

			.c5ab_col_<?php echo $i?> { width: <?php echo ($i*100)/C5BP_COLUMNS_COUNT ?>%; }
			<?php

			/*for ($sub_i = 1; $sub_i <= C5BP_COLUMNS_COUNT; $sub_i++) {
				$value = round($sub_i*$i/C5BP_COLUMNS_COUNT);
				if($value == 0){
					$value =1;
				}
	...
	function presets() {
		...
		foreach ($presets as $preset) {
			if ($preset['class'] != 'theme-template') {
				$classes = explode('_' , $preset['class']);
				$new_classes = array();
				$bool = true;
				foreach ($classes as $value) {
					$value_new = $value/12*C5BP_COLUMNS_COUNT;
				...
				echo '<span class="c5ab_preset_'.$preset['class'].' '.$preset['icon'].' c5ab-full-i" title="'.$preset['title'].'"></span>';
			}
	...
	?>
		<div class="c5ab-bg-wrap c5ab-grid">
			<div class="c5ab-grid-row">
				<?php
				for ($i = 0; $i <= C5BP_COLUMNS_COUNT; $i++) {
					?>
