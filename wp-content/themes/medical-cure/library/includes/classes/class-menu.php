        ...
        <li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes); ?>">
            <dl class="menu-item-bar">
                <dt class="menu-item-handle">
                    ...
                    ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'medical-cure'); ?>">&#8593;</abbr></a>
                    |
                    <a href="<?php
                    echo wp_nonce_url(
                    add_query_arg(
            ...
            ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'medical-cure'); ?>">&#8595;</abbr></a>
        </span>
        <a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php esc_attr_e('Edit Menu Item', 'medical-cure'); ?>" href="<?php
...
<div class="menu-item-settings clearfix" id="menu-item-settings-<?php echo $item_id; ?>">
    ...
    <div class="format-setting-inner field-custom description description-wide">
        <?php  esc_html_e('Choose an Icon', 'medical-cure'); ?><br/>
        <input type="hidden" name="menu-item-subtitle[<?php echo $item_id; ?>]" id="edit-menu-item-subtitle-<?php echo $item_id; ?>" value="<?php echo esc_attr($item->subtitle); ?>" class="c5-icon-list-value" />

        <p class="c5-icon-select"><span class="<?php echo esc_attr($item->subtitle); ?>"><?php echo esc_attr($item->subtitle); ?></span></p>
        <div class="c5-icons-wrap"></div>
    ...
    if (!is_object($thisCat)) {
        ?>
            <p class="field-custom description description-wide">
                <label for="edit-menu-item-wide_menu-<?php echo $item_id; ?>">
                    <?php echo 'Wide Menu'; ?><br />
                    <?php
                    ...
                    ?>
                    <select id="edit-menu-item-wide_menu-<?php echo $item_id; ?>" class="widefat code edit-menu-item-custom" name="menu-item-wide_menu[<?php echo $item_id; ?>]">
                        <?php
                        foreach ($options as $key => $value) {
                            $selected = '';
                            if (esc_attr($item->wide_menu) == $key) {
...
class c5_custom_menu {
    ...
    */
    function add_custom_nav_fields($menu_item) {
        ...
        $menu_item->wide_menu = get_post_meta($menu_item->ID, '_menu_item_wide_menu', true);
    ...
    */
    function update_custom_nav_fields($menu_id, $menu_item_db_id, $args) {
        ...
        */
        ...
        if (isset($_REQUEST['menu-item-wide_menu'])) {
            if (is_array($_REQUEST['menu-item-wide_menu'])) {
                if (isset($_REQUEST['menu-item-wide_menu'][$menu_item_db_id])) {
                    $template_value = $_REQUEST['menu-item-wide_menu'][$menu_item_db_id];
                    update_post_meta($menu_item_db_id, '_menu_item_wide_menu', $template_value);
                }
            }
        }
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        if (is_object($thisCat)) {
            ...
            if ($item->template != '' && $item->template != 'no') {
                $li_class = ' c5-mega-menu-template-li ' . $item->object_id . '-' . $item->object;
                $a_class = ' c5-mega-menu-a';
                $a_data = '<span class="c5-mega-data" data-mega-menu-id="' . $item->object_id . '#' . $item->object . '#' . $item->template .'"></span>';
            }
            */
        ...
        if ($item->wide_menu != '' && $item->wide_menu != 'no') {
            $li_class .= ' c5-mega-menu-li';
        }
        ...
        $output .= $indent . '<li   ' . $value . $class_names . '>' . $a_data;
        ...
        $item_output .= '<a' . $attributes . '  class="' . $a_class . ' " >';
        if($depth == 0){
            $item_output .= '<span class="original"><i class="'.$category_icon.' ' . esc_attr($item->subtitle) . '"></i>' ;
            $item_output .= $args->link_before . $prepend . apply_filters('the_title', $item->title, $item->ID)  . $append . $args->link_after;
            $item_output .= '</span>';
            $item_output .= '<span class="hover"><i class="'.$category_icon.' ' . esc_attr($item->subtitle) . '"></i>' ;
            $item_output .= $args->link_before . $prepend . apply_filters('the_title', $item->title, $item->ID)  . $append . $args->link_after;
            $item_output .= '</span>';
        }else{
            $item_output .= '<span class="'.$category_icon.' ' . esc_attr($item->subtitle) . '"></span>' ;
            $item_output .= $args->link_before . $prepend . apply_filters('the_title', $item->title, $item->ID)  . $append . $args->link_after;
        }
        ...
        /*
        if ($item->template != '' && $item->template != 'no') {
            $item_output .= '<div class="c5-mega-menu-wrap" ><div class="mid-page"><span class=" fa fa-circle-o-notch c5-loading fa-spin"></span></div></div>';
        }
        */
...
function c5ab_menu_mega_menu() {
    ...
    if ( $the_query->have_posts() ) {
        ...
        $atts = array(
            'meta_data' => ot_get_option('meta_data'),
            'c5_date_format' => ot_get_option('c5_date_format')
        );
        while ( $the_query->have_posts() ) {
            ...
            $open = false;
            if($current_post == 0){
                $return .=  '<div class="col-sm-4">' .$post_obj->get_post_thumb_tall($atts) .'</div>';
            }else {
                if ($current_post == 1) {
                    $return .=  '<div class="col-sm-4">';
                    $open = true;
                }elseif ($current_post == 4) {
