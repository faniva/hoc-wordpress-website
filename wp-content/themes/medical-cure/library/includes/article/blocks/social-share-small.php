...
if (!empty($valid_meta_data)) {
    ...
    foreach ($valid_meta_data as $single_data) {
        ...
        switch ($single_data) {
            case 'facebook':
                $current_data .= '<li><a class="c5-social facebook" href="https://www.facebook.com/sharer.php?u='. urlencode(get_permalink()) .'"><span class="fa fa-facebook"></span> '.esc_html__('Share','medical-cure').'</a></li>';

                break;

            case 'twitter':
                $current_data .= '<li><a class="c5-social twitter" href="https://twitter.com/share?text=' . get_the_title()  .'"><span class="fa fa-twitter"></span> '.esc_html__('Tweet','medical-cure').'</a></li>';
